...
def render_reserves(data, members=None):
    st.subheader("Reserves & loss ratio · membership premiums")
    ...
    if members is None:
        st.info("Open the updated membership workbook in Data setup. Premiums are read only from its annual premium column.")
        return
    if "annual premium" not in members.columns:
        st.error("The membership workbook does not contain the required annual premium column. Please select the latest membership file.")
        return
    try:
        currencies = sorted(set(data["CLAIM_CURRECY"].str.upper()))
        assumed_currency = st.selectbox("Annual premium currency · membership assumption", currencies, index=currencies.index("USD") if "USD" in currencies else 0)
        st.info("Premium source: the annual premium column in the uploaded membership workbook. Monthly earned premium = annual premium ÷ 12 × covered days ÷ days in month. Joining and policy-end dates are inclusive.")
        ...
        st.error("Membership premiums could not be used: " + str(exc))
        return
    ...
    st.caption("Premium amounts come only from the membership workbook. Paid completion remains an editable dashboard assumption; 85% is an arbitrary starting value.")
    ...
    st.subheader("Claims amount bridge")
    w, x, y, z = st.columns(4)
    w.metric("Fictional earned premium · " + currency, "Incomplete" if missing else f"{premium:,.2f}")
    x.metric("Paid claims · " + currency, f"{paid:,.2f}")
    y.metric("Estimated unpaid reserve · " + currency, f"{reserve:,.2f}")
    z.metric("Estimated ultimate claims · " + currency, f"{ultimate:,.2f}")
    available = not missing and premium > 0
    paid_ratio = paid / premium if available else float("nan")
    reserve_ratio = reserve / premium if available else float("nan")
    ultimate_ratio = ultimate / premium if available else float("nan")
    st.subheader("Ratio bridge")
    x, y, z = st.columns(3)
    x.metric("Paid claims ratio", f"{paid_ratio:.2%}" if available else "N/A")
    y.metric("Reserve ratio", f"{reserve_ratio:.2%}" if available else "N/A")
    z.metric("Estimated ultimate claims ratio", f"{ultimate_ratio:.2%}" if available else "N/A")
    if available:
        st.info(
            f"{currency} {paid:,.2f} paid + {currency} {reserve:,.2f} estimated reserve "
            f"= {currency} {ultimate:,.2f} estimated ultimate claims. "
            f"Against {currency} {premium:,.2f} fictional earned premium: "
            f"{paid_ratio:.2%} + {reserve_ratio:.2%} = {ultimate_ratio:.2%}."
        )
        if completion < 100:
            st.caption(
                f"Why the estimated ultimate ratio is higher: at {completion:.1f}% paid completion, "
                f"estimated ultimate claims = paid claims ÷ {completion:.1f}%. "
                "A positive unpaid reserve is therefore added to paid claims."
            )
        else:
            st.caption("At 100% paid completion, the estimated reserve is zero and the two claims ratios are equal.")
    ...
    with st.expander("Calculation and coverage assumptions", expanded=True):
        st.write("Estimated ultimate claims = paid claims ÷ assumed paid-completion proportion. Estimated unpaid reserve = ultimate − paid.")
        st.write("Paid claims ratio = paid claims ÷ fictional earned premium. Reserve ratio = estimated unpaid reserve ÷ fictional earned premium. Estimated ultimate claims ratio = paid claims ratio + reserve ratio.")
        st.write("Because this model only allows completion between 1% and 100%, its reserve cannot be negative. The estimated ultimate claims ratio must therefore be equal to or higher than the paid claims ratio. It is equal only at 100% completion.")
...
def main():
    ...
    st.sidebar.caption("Version 0.10 · Fictional data prototype")
    st.title(TITLE)
    st.subheader(page)
    ...
    if page == "About the data":
        ...
        st.write("The Reserves & loss ratio and Premiums & paid ratio pages read fictional premiums directly from the annual premium column in the uploaded membership workbook. No separate premium workbook is used. Claims per covered life, annual benefit utilisation and a separately estimated IBNR are not calculated.")
